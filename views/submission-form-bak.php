...
<div id="fep-new-post">
	...
	<form id="fep-submission-form">
		<label for="fep-post-title"><?php _e('Title', 'lapor-hoax'); ?></label>
		<input type="text" name="post_title" id="fep-post-title" value="<?php echo ($post) ? $post['title'] : ''; ?>">
		<label for="fep-post-content"><?php _e('Content', 'lapor-hoax'); ?></label>
		<?php
		...
		?>
		<?php if (!$lh_misc['disable_author_bio']): ?>
			<label for="fep-about"><?php _e('Author Bio', 'lapor-hoax'); ?></label>
		...
		<?php endif; ?>
		<label for="fep-category"><?php _e('Category', 'lapor-hoax'); ?></label>
		<?php wp_dropdown_categories(array('id' => 'fep-category', 'hide_empty' => 0, 'name' => 'post_category', 'orderby' => 'name', 'selected' => $post['category'], 'hierarchical' => true, 'show_option_none' => __('None', 'lapor-hoax'))); ?>
		...
		<div id="fep-featured-image">
			...
			<a id="fep-featured-image-link" href="#"><?php _e('Choose Featured Image', 'lapor-hoax'); ?></a>
		...
		<button type="button" id="fep-submit-post" class="active-btn"><?php _e('Submit', 'lapor-hoax'); ?></button>
